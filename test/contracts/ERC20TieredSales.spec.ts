...
import { ERC20MintableOwnable } from '../../src/typechain/ERC20MintableOwnable';
...
const deployERC20WithSales = async ({
  ...
  return deployDiamond({
    ...
    initializations: [
      ...
      {
        facet: 'ERC20SupplyOwnable',
        function: 'setMaxSupply',
        args: [ethers.utils.parseEther('8000')],
      },
      {
        facet: 'ERC20MetadataOwnable',
...
describe('ERC20 Tiered Sales', function () {
  it('should return facets', async function () {
    ...
    expect(facets.length).to.be.equal(11);
  });
  ...
  it('should mint by tier multiple times until reached max allowance in allowlist', async function () {
    ...
    const mkt = generateAllowlistMerkleTree([
      {
        ...
        maxAllowance: ethers.utils.parseEther('3'),
      },
      {
        address: userB.signer.address,
        maxAllowance: ethers.utils.parseEther('2'),
      },
      {
        address: userC.signer.address,
        maxAllowance: ethers.utils.parseEther('4'),
      },
    ]);
    ...
    expect(await erc20Facet.balanceOf(userA.signer.address)).to.equal(ethers.utils.parseEther('1.75'));
  ...
  it('should mint by tier when 1 tier, no allowlist, with native currency', async function () {
    ...
    await tieredSalesFacet.connect(userA.signer).mintByTier(0, ethers.utils.parseEther('2'), 0, [], {
      value: utils.parseEther('0.12'),
    });

    expect(await erc20Facet.balanceOf(userA.signer.address)).to.be.equal(ethers.utils.parseEther('2'));
  });
  ...
  it('should fail to mint when much higher than allocation', async function () {
    const { userA } = await setupTest();
    ...
    await expect(
      tieredSalesFacet.connect(userA.signer).mintByTier(0, ethers.constants.MaxUint256, 0, [], {
        value: utils.parseEther('1'),
      }),
    ).to.be.revertedWith('EXCEEDS_MAX');
  });

  it('should mint highest possible amount', async function () {
    const { userA } = await setupTest();
      ...
      tiers: [
        {
          ...
          currency: ZERO_ADDRESS,
          maxPerWallet: ethers.constants.MaxUint256,
          merkleRoot: ZERO_BYTES32,
          price: 0,
          reserved: 0,
          maxAllocation: ethers.constants.MaxUint256,
        },
      ],
      initializations: [
        {
    ...
    const tieredSalesFacet = await hre.ethers.getContractAt<ERC20TieredSales>('ERC20TieredSales', diamond.address);
    const erc20Base = await hre.ethers.getContractAt<ERC20Base>('ERC20Base', diamond.address);

    await tieredSalesFacet.connect(userA.signer).mintByTier(0, ethers.constants.MaxUint256, 0, [], {
      value: utils.parseEther('1'),
    });

    expect(await erc20Base.balanceOf(userA.signer.address)).to.equal(ethers.constants.MaxUint256);
  });

  it('should require payment even when minting dust', async function () {
    const { userA } = await setupTest();
    ...
    await expect(
      tieredSalesFacet.connect(userA.signer).mintByTier(0, '10', 0, [], {
  ...
  it('should fail when minting a tier when max per wallet is reached', async function () {
    ...
    await tieredSalesFacet.connect(userA.signer).mintByTier(0, utils.parseEther('5'), 0, [], {
      value: utils.parseEther('0.30'),
    });

    await expect(
      tieredSalesFacet.connect(userA.signer).mintByTier(0, utils.parseEther('1'), 0, [], {
        value: utils.parseEther('0.06'),
      }),
    ).to.be.revertedWith('EXCEEDS_MAX');
  });
  ...
  it('should fail when minting a tier and amount + prev mints is asking for more than remaining per-wallet allocation', async function () {
    ...
    await tieredSalesFacet.connect(userA.signer).mintByTier(0, utils.parseEther('4'), 0, [], {
      value: utils.parseEther('0.24'),
    });

    await expect(
      tieredSalesFacet.connect(userA.signer).mintByTier(0, utils.parseEther('2'), 0, [], {
        value: utils.parseEther('0.12'),
      }),
    ).to.be.revertedWith('EXCEEDS_MAX');
  });
  ...
  it('should mint by tier when multiple tiers and have dedicated max per wallet', async function () {
    ...
    const mkt = generateAllowlistMerkleTree([
      {
        address: userA.signer.address,
        maxAllowance: utils.parseEther('3'),
      },
      {
        address: userB.signer.address,
        maxAllowance: utils.parseEther('2'),
      },
      {
        address: userC.signer.address,
        maxAllowance: utils.parseEther('4'),
      },
    ]);
      ...
      tiers: [
        {
          ...
          currency: ZERO_ADDRESS,
          maxPerWallet: utils.parseEther('3'),
          merkleRoot: mkt.getHexRoot(),
          price: utils.parseEther('0.06'),
          reserved: 0,
        ...
        },
        {
          ...
          currency: ZERO_ADDRESS,
          maxPerWallet: utils.parseEther('5'),
          merkleRoot: ZERO_BYTES32,
          price: utils.parseEther('0.2'),
          reserved: 0,
          maxAllocation: utils.parseEther('5000'),
        },
        {
          ...
          currency: ZERO_ADDRESS,
          maxPerWallet: utils.parseEther('1'),
          merkleRoot: ZERO_BYTES32,
    ...
    await tieredSalesFacet.connect(userB.signer).mintByTier(1, utils.parseEther('5'), 0, [], {
      value: utils.parseEther('1'),
    });

    await expect(
      tieredSalesFacet.connect(userB.signer).mintByTier(1, utils.parseEther('1'), 0, [], {
        value: utils.parseEther('0.6'),
      }),
    ).to.be.revertedWith('EXCEEDS_MAX');

    await tieredSalesFacet.connect(userB.signer).mintByTier(2, utils.parseEther('1'), 0, [], {
      value: utils.parseEther('0.01'),
    });

    await expect(
      tieredSalesFacet.connect(userB.signer).mintByTier(2, utils.parseEther('1'), 0, [], {
        value: utils.parseEther('0.01'),
      }),
    ).to.be.revertedWith('EXCEEDS_MAX');
  });
  ...
  it('should fail to mint by tier if remaining allocation is fully reserved (example A)', async function () {
    ...
    const erc20Base = await hre.ethers.getContractAt<IERC20>('IERC20', diamond.address);
    const tieredSalesFacet = await hre.ethers.getContractAt<ERC20TieredSales>('ERC20TieredSales', diamond.address);
    ...
    expect(await erc20Base.connect(userB.signer).totalSupply()).to.be.equal(5);
    ...
    expect(await erc20Base.connect(userB.signer).totalSupply()).to.be.equal(7);
  ...
  it('should fail to mint if collection supply is reached', async function () {
    ...
    const tieredSalesFacet = await hre.ethers.getContractAt<ERC20TieredSales>('ERC20TieredSales', diamond.address);
    const mintableOwnableFacet = await hre.ethers.getContractAt<ERC20MintableOwnable>(
      'ERC20MintableOwnable',
      diamond.address,
    );
  ...
  // TODO Should we override setMaxSupply when using TieredSales in 20 cleanly?
  it.skip('should fail when total remaining supply become less than remaining reserved spots', async function () {
    ...
    const diamond = await deployERC20WithSales({
      tiers: [
        ...
        },
        {
          ...
          currency: ZERO_ADDRESS,
          maxPerWallet: utils.parseEther('2'),
          merkleRoot: ZERO_BYTES32,
          price: utils.parseEther('1'),
          reserved: utils.parseEther('2'),
          maxAllocation: utils.parseEther('5000'),
        },
        {
          start: Math.floor(+new Date() / 1000) - 4 * 24 * 60 * 60,
      ...
      initializations: [
        {
          facet: 'ERC20SupplyOwnable',
          function: 'setMaxSupply',
          args: [utils.parseEther('10')],
        },
      ],
    });
    const tieredSalesFacet = await hre.ethers.getContractAt<ERC20TieredSales>('ERC20TieredSales', diamond.address);
    const supplyOwnableFacet = await hre.ethers.getContractAt<ERC20SupplyOwnable>(
      'ERC20SupplyOwnable',
      diamond.address,
    );
    ...
    expect([
      ...
      utils.parseEther('8').toString(),
      utils.parseEther('7').toString(),
      utils.parseEther('5').toString(),
    ]);

    await tieredSalesFacet.connect(userA.signer).mintByTier(0, utils.parseEther('1'), utils.parseEther('1'), [], {
      value: utils.parseEther('1'),
    });
    await tieredSalesFacet.connect(userA.signer).mintByTier(1, utils.parseEther('2'), utils.parseEther('2'), [], {
      value: utils.parseEther('2'),
    });
    ...
    // await expect(supplyOwnableFacet.connect(deployer.signer).setMaxSupply(utils.parseEther('33'))).to.be.revertedWith(
    //   'LOWER_THAN_RESERVED',
    // );
  });
  ...
  // TODO Should we enforce max supply in tiered sales extension of 20?
